    ...
    return """
         <html><body>
             ...
             <form action="/temp_out">
                 What month? <input type='number' name='month'><br>
                 What day? <input type='number' name='day'><br>
...
def temp_out():    # Create our session (link) from Python to the DB
    ...
    session = Session(engine)
    result = session.query(func.min(Measurement.tobs), func.avg(Measurement.tobs), func.max(Measurement.tobs)).filter(func.strftime("%Y-%m-%d",Measurement.date) >= bd_object).all()
    session.close()
